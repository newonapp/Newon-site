...
생성된 **`publish/`** 안에 `index.html`, CSS, **`assets` 이미지 전부**가 들어 있습니다.

- 올릴 때 빠짐 방지: 저장소 루트 **`업로드-체크리스트.txt`** 를 참고하세요.
- **폴더째 FTP**(FileZilla 등): `publish` 폴더 통째로 드래그하면 하위 파일이 한 번에 올라갑니다.
- **ZIP을 허용하는 호스만**: 같은 명령으로 루트에 **`newon-site.zip`** 도 생성됩니다 → 업로드 후 **압축 풀기**하면 `index.html`·`assets` 가 한 번에 배치됩니다.
